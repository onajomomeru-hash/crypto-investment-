...
if not RAW:
    raise SystemExit("DATABASE_URL is not set. Add your Neon connection string.")
...
CREATE INDEX IF NOT EXISTS idx_daily_day ON daily_results(day DESC);
"""
...
def sync_balance():
    """Portfolio balance always mirrors the most recent published day."""
    latest = one("SELECT end_balance::float AS b FROM daily_results ORDER BY day DESC LIMIT 1")
    val = str(latest["b"]) if latest else "0"
    run("""INSERT INTO settings (key, value) VALUES ('portfolio_balance', :v)
           ON CONFLICT (key) DO UPDATE SET value = :v""", {"v": val})
...
def init_db():
    ...
    if admin_email and admin_pw:
        existing = one("SELECT id FROM users WHERE email = :e", {"e": admin_email})
        if not existing:
            run("INSERT INTO users (email, password_hash, is_admin) VALUES (:e, :p, TRUE)",
                {"e": admin_email, "p": generate_password_hash(admin_pw)})
            print(f"[init] admin account created: {admin_email}")
        else:
            run("UPDATE users SET is_admin = TRUE WHERE email = :e", {"e": admin_email})
...
def make_session(user_id):
    ...
    run("INSERT INTO sessions (token, user_id, expires_at) VALUES (:t, :u, :x)",
        {"t": tok, "u": user_id, "x": dt.datetime.now(dt.timezone.utc) + dt.timedelta(days=SESSION_DAYS)})
    return tok
...
def summary():
    hist = rows("""SELECT id, day::text AS day, start_balance::float AS start_balance,
                   ...
                   FROM daily_results ORDER BY day DESC LIMIT 365""")
    agg = one("""SELECT COALESCE(SUM(pnl),0)::float AS total,
                        COUNT(*) AS n,
                        COUNT(*) FILTER (WHERE pnl > 0) AS wins
                 FROM daily_results""")
    bal = one("SELECT value FROM settings WHERE key = 'portfolio_balance'")
    return jsonify({
        "balance": float(bal["value"]) if bal else 0.0,
        "total_pnl": agg["total"] if agg else 0.0,
        "days": agg["n"] if agg else 0,
...
def login():
    ...
    return jsonify({"token": make_session(u["id"]), "email": u["email"],
                    "is_admin": u["is_admin"]})
...
def create_request():
    ...
    if one("SELECT id FROM requests WHERE user_id = :u AND status = 'pending'",
           {"u": g.user["id"]}):
...
def admin_requests():
    pending = rows("""SELECT r.id, r.kind, r.amount::float AS amount, r.address,
                             r.status, r.created_at::text AS created_at, u.email
                      FROM requests r JOIN users u ON u.id = r.user_id
                      WHERE r.status = 'pending' ORDER BY r.id""")
    recent = rows("""SELECT r.id, r.kind, r.amount::float AS amount, r.status,
                            r.created_at::text AS created_at, u.email
                     FROM requests r JOIN users u ON u.id = r.user_id
                     WHERE r.status <> 'pending' ORDER BY r.decided_at DESC LIMIT 25""")
...
def admin_trade():
    ...
    prev = one("""SELECT end_balance::float AS b FROM daily_results
                  WHERE day < :d ORDER BY day DESC LIMIT 1""", {"d": day})
    existing = one("SELECT start_balance::float AS b FROM daily_results WHERE day = :d", {"d": day})
    start = existing["b"] if existing else (prev["b"] if prev else 0.0)

    run("""INSERT INTO daily_results (day, start_balance, end_balance, pnl, note)
           VALUES (:d, :s, :e, :p, :n)
           ON CONFLICT (day) DO UPDATE
           SET start_balance = :s, end_balance = :e, pnl = :p, note = :n""",
        {"d": day, "s": start, "e": end_bal, "p": pnl, "n": note})
    sync_balance()
    return jsonify({"ok": True})

@app.delete("/api/admin/trade/<day>")
@require_admin
def admin_delete_trade(day):
    run("DELETE FROM daily_results WHERE day = :d", {"d": day})
    sync_balance()
    return jsonify({"ok": True})
...
def admin_users():
    return jsonify({"users": rows("""
        SELECT u.id, u.email, u.is_admin, u.created_at::text AS created_at,
               COALESCE(SUM(CASE WHEN r.status='approved' AND r.kind='deposit'
                                 THEN r.amount END), 0)::float AS deposited,
               COALESCE(SUM(CASE WHEN r.status='approved' AND r.kind='withdraw'
                                 THEN r.amount END), 0)::float AS withdrawn
